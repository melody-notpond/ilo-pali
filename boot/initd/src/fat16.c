// TODO: MOVE THIS TO LIBRARY SHARED BETWEEN KERNEL AND USERSPACE
...
fat16_fs_t verify_initrd(void* start, void* end) {
    ...
    if (signature != SIGNATURE) {
        uart_puts("[verify_initrd] invalid fat16 signature");
        return (fat16_fs_t) { 0 };
    }
    ...
    if (fat_size == 0 || total_sector_count == 0) {
        uart_puts("[verify_initrd] one of fat size or total sector count is 0");
        return (fat16_fs_t) { 0 };
    }
    ...
    if (cluster_count < FAT12_LIMIT || cluster_count >= FAT16_LIMIT) {
        uart_puts("[verify_initrd] fat file system is not FAT16");
        return (fat16_fs_t) { 0 };
    }
    ...
    if (fat_end != end) {
        uart_puts("[verify_initrd] fat end does not equal initrd end");
        return (fat16_fs_t) { 0 };
    }
...
void* read_file_full(fat16_fs_t* fs, char* name, size_t* size_ptr) {
    ...
    if (entry == NULL)
        return NULL;
    // TODO: use malloc instead of this
    void* data = page_alloc((entry->file.file_size + PAGE_SIZE - 1) / PAGE_SIZE, PAGE_PERM_READ | PAGE_PERM_WRITE);
    if (data == NULL)
        return NULL;
