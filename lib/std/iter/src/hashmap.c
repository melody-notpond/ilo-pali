...
hashmap_t* create_hashmap(alloc_t* allocator, size_t key_size, size_t val_size) {
    hashmap_t* hashmap = alloc(allocator, sizeof(hashmap_t));
    *hashmap = alloc(allocator, sizeof(struct s_hashmap) + INITIAL_HASHMAP_SIZE * sizeof(struct s_hashmap_bucket));
...
void hashmap_grow(hashmap_t* hashmap) {
    ...
    alloc_t* allocator = (*hashmap)->alloc;
    hashmap_t new = alloc(allocator, sizeof(struct s_hashmap) + sizeof(struct s_hashmap_bucket) * (*hashmap)->cap * 2);
    *new = (struct s_hashmap) {
    ...
    for (size_t i = 0; i < (*hashmap)->cap; i++) {
        ...
        for (size_t j = 0; j < bucket->len; j++) {
            ...
            if (new_bucket->len == 0) {
                new_bucket->keys = alloc(allocator, key_size * HASHMAP_MAX_COLLISION_COUNT);
                new_bucket->values = alloc(allocator, val_size * HASHMAP_MAX_COLLISION_COUNT);
            }
    ...
    for (size_t i = 0; i < (*hashmap)->cap; i++) {
        dealloc(allocator, (*hashmap)->ring[i].keys);
        dealloc(allocator, (*hashmap)->ring[i].values);
    }

    dealloc(allocator, *hashmap);
    *hashmap = new;
}
...
void hashmap_insert(hashmap_t* hashmap, void* key, void* value) {
    ...
    while (true) {
        ...
        if (bucket->len < HASHMAP_MAX_COLLISION_COUNT) {
            if (bucket->len == 0) {
                bucket->keys = alloc(allocator, key_size * HASHMAP_MAX_COLLISION_COUNT);
                bucket->values = alloc(allocator, val_size * HASHMAP_MAX_COLLISION_COUNT);
            }
...
void hashmap_remove(hashmap_t* hashmap, void* key) {
    ...
    if (bucket->len == 0) {
        dealloc(allocator, bucket->keys);
        bucket->keys = NULL;
        dealloc(allocator, bucket->values);
        bucket->values = NULL;
    }
}
